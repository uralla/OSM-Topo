...
class HighwayTunnelTests(unittest.TestCase):
    ...
    def test_tunnel_overview_thresholds_are_softer_than_ordinary_roads(self) -> None:
        text = self._text()
        self.assertIn("highway=secondary & tunnel=yes & length()>250 [0x10e04 resolution 18-19 continue]", text)
        self.assertIn("highway=tertiary & tunnel=yes & length()>250 [0x10e04 resolution 19-22 continue]", text)
        self.assertIn("highway=track & tracktype=grade1 & tunnel=yes & length()>50 [0x10e04 resolution 21-23 continue]", text)
        self.assertIn("highway=track & tracktype!=grade1 & tunnel=yes & length()>50 [0x10e04 resolution 22-23 continue]", text)
        self.assertIn("highway=cycleway & tunnel=yes & length()>100 [0x10e04 resolution 22-23 continue]", text)
