...
class LineFallbackCleanupTests(unittest.TestCase):
    ...
    def test_special_purpose_highways_do_not_enter_generic_motor_routing(self) -> None:
        lines = LINES.read_text(encoding='utf-8')
        fallback = "highway=* & area!=yes & highway!=path & highway!=steps & highway!=footway & highway!=track & highway!=cycleway & highway!=service [0x07 road_class=0 road_speed=0 resolution 24]"
        for rule in (
    ...
    def test_indoor_corridor_is_foot_routable_not_generic_motor_road(self) -> None:
        lines = LINES.read_text(encoding='utf-8')
    ...
    def test_removed_highways_cannot_leak_active_overlays(self) -> None:
        ...
        bridge = "highway=* & bridge=* & bridge!=no & bridge!=proposed & bridge!=abandoned & highway!=pedestrian & highway!=footway & highway!=path"
        ...
        self.assertIn("bridge=* & bridge!=no & bridge!=proposed & bridge!=abandoned & (highway=pedestrian | highway=footway | highway=path)", lines)
